...
class SSD300:
    def __init__(self, tf_sess, isTraining, class_size=21, graph_config='ssd'):
        # tensorflow session
        self.sess = tf_sess
        # 是否训练
        ...
        if graph_config.lower() == 'ssd':
            # 初始化Tensorflow Graph
            self.generate_graph()
        else:
            self.generate_graph_dsod()

    def generate_graph(self):
        # 输入数据
        self.input = tf.placeholder(shape=[None, self.img_size[0], self.img_size[1], 3], dtype=tf.float32,
                                    name='input_image')
        ...
        # 输入真实数据
        self.groundtruth_class = tf.placeholder(shape=[None, self.all_default_boxs_len], dtype=tf.int32,
                                                name='groundtruth_class')
        self.groundtruth_location = tf.placeholder(shape=[None, self.all_default_boxs_len, 4], dtype=tf.float32,
                                                   name='groundtruth_location')
        self.groundtruth_positives = tf.placeholder(shape=[None, self.all_default_boxs_len], dtype=tf.float32,
                                                    name='groundtruth_positives')
        self.groundtruth_negatives = tf.placeholder(shape=[None, self.all_default_boxs_len], dtype=tf.float32,
                                                    name='groundtruth_negatives')

        # 损失函数
        self.groundtruth_count = tf.add(self.groundtruth_positives, self.groundtruth_negatives)
        self.softmax_cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.feature_class,
                                                                                    labels=self.groundtruth_class)
        self.loss_location = tf.div(tf.reduce_sum(tf.multiply(
            tf.reduce_sum(self.smooth_L1(tf.subtract(self.groundtruth_location, self.feature_location)),
                          reduction_indices=2), self.groundtruth_positives), reduction_indices=1),
            tf.reduce_sum(self.groundtruth_positives, reduction_indices=1))
    ...
    def run(self, input_images, actual_data, file_list):
        # 训练部分
        if self.isTraining:
            if actual_data is None or None in actual_data:
                print('has None item')
                print(file_list)
            ...
            f_class, f_location = self.sess.run([self.feature_class, self.feature_location],
            ...
            with tf.control_dependencies([self.feature_class, self.feature_location]):
                ...
                gt_class, gt_location, gt_positives, gt_negatives = self.generate_groundtruth_data(actual_data,
                                                                                                   file_list)
                ...
                with tf.control_dependencies([self.train]):
                    loss_all, loss_location, loss_class = self.sess.run(
                        [self.loss_all, self.loss_location, self.loss_class], feed_dict={
                            self.input: input_images,
                            self.groundtruth_class: gt_class,
                            self.groundtruth_location: gt_location,
                            self.groundtruth_positives: gt_positives,
        ...
        # 检测部分
            ...
            for i in range(top_shape[0]):
                ...
                for j in range(top_shape[1]):
                    ...
                    if p_location[0] < 0 or p_location[1] < 0 or p_location[2] < 0 or p_location[3] < 0 or p_location[
                        2] == 0 or p_location[3] == 0:
                        continue
                    is_box_filter = False
                    for f_index in range(len(item_img_class)):
                        if self.jaccard(p_location, item_img_location[f_index]) > 0.3 and p_class == item_img_class[
                            f_index]:
                            is_box_filter = True
                            break
    ...
    def batch_normalization(self, input, name):
        with tf.variable_scope(name):
            ...
            moving_mean = tf.get_variable(name + '_mean', bn_input_shape[-1:], initializer=tf.zeros_initializer,
                                          trainable=False)
            moving_variance = tf.get_variable(name + '_variance', bn_input_shape[-1:], initializer=tf.ones_initializer,
                                              trainable=False)
    ...
    # 整理生成groundtruth数据
    def generate_groundtruth_data(self, input_actual_data, file_list):
        ...
        for img_index in range(input_actual_data_len):
            try:
                ...
                print(e)
                print("problematic_file: " + file_list[img_index])
    ...
    def generate_graph_dsod(self):
        self.input = tf.placeholder(shape=[None, self.img_size[0], self.img_size[1], 3], dtype=tf.float32,
                                    name='input_image')
        net = conv2d(self.input, 64, (3, 3), (2, 2), padding='SAME', name='stem1')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='stem1_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 64, (3, 3), (1, 1), padding='SAME', name='stem2')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='stem2_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 128, (3, 3), (1, 1), padding='SAME', name='stem3')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='stem3_bn')
        net = tf.nn.relu(net)
        net = max_pooling2d(net, pool_size=2, strides=(2, 2), name='stem3_pool')
        # dense block 1
        net = self.__denseblock(net, nblock=6, steps=48, firstchannel=192, is_train=self.isTraining, name='denseblock0')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='denseblock0_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 416, (1, 1), (1, 1), padding='SAME', name='denseblock0_cnn')
        net = max_pooling2d(net, pool_size=2, strides=(2, 2), name='denseblock0_pool')
        # dens block 2
        net = self.__denseblock(net, nblock=8, steps=48, firstchannel=192, is_train=self.isTraining, name='denseblock1')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='denseblock1_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 800, (1, 1), (1, 1), padding='SAME', name='denseblock1_cnn')
        netfirst = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_first_bn')
        netfirst = tf.nn.relu(netfirst)
        net = max_pooling2d(net, pool_size=2, strides=(2, 2), name='denseblock2_pool1')
        # dense block 3
        net = self.__denseblock(net, nblock=8, steps=48, firstchannel=192, is_train=self.isTraining, name='denseblock2')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='denseblock2_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 1184, (1, 1), (1, 1), padding='SAME', name='denseblock2_cnn')
        # dense block 4
        net = self.__denseblock(net, nblock=8, steps=48, firstchannel=192, is_train=self.isTraining, name='denseblock3')
        net = batch_normalization(net, training=self.isTraining, trainable=True, name='denseblock3_bn')
        net = tf.nn.relu(net)
        net = conv2d(net, 256, (1, 1), (1, 1), padding='SAME', name='denseblock2_cnna')
        netpl = max_pooling2d(netfirst, pool_size=2, strides=(2, 2), name='First_pool')
        netpl = batch_normalization(netpl, training=self.isTraining, trainable=True, name='First_bn')
        netpl = tf.nn.relu(netpl)
        netpl = conv2d(netpl, 256, (1, 1), (1, 1), padding='SAME', name='denseblock2_cnnb')
        net = tf.concat([net, netpl], -1, "Second_Cat")
        netsecond = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_second_bn')
        netsecond = tf.nn.relu(netsecond)

        # dense block pl
        net = self.__denseblockpl(net, steps=256, firstchannel=256, training=self.isTraining, name='denseplz1')
        netthird = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_third_bn')
        netthird = tf.nn.relu(netthird)

        # dense block pl
        net = self.__denseblockpl(net, steps=128, firstchannel=128, training=self.isTraining, name='denseplz2')
        netfourth = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_fourth_bn')
        netfourth = tf.nn.relu(netfourth)

        # dense block pl
        net = self.__denseblockpl(net, steps=128, firstchannel=128, training=self.isTraining, name='denseplz3')
        netfifth = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_fifth_bn')
        netfifth = tf.nn.relu(netfifth)

        # dense block fin
        net = self.__denseblockfin(net, step=128, firstchannel=128, training=self.isTraining, name='denseplz4')
        netsixth = batch_normalization(net, training=self.isTraining, trainable=True, name='feature_sixth_bn')
        netsixth = tf.nn.relu(netsixth)

        outfirst = conv2d(netfirst, self.default_box_size[0] * (self.classes_size + 4), (3, 3), (1, 1), padding='SAME',
                          name='firstout')
        outsecond = conv2d(netsecond, self.default_box_size[1] * (self.classes_size + 4), (3, 3), (1, 1),
                           padding='SAME',
                           name='secondout')
        outthird = conv2d(netthird, self.default_box_size[2] * (self.classes_size + 4), (3, 3), (1, 1), padding='SAME',
                          name='thirdout')
        outfourth = conv2d(netfourth, self.default_box_size[3] * (self.classes_size + 4), (3, 3), (1, 1),
                           padding='SAME',
                           name='fourthout')
        outfifth = conv2d(netfifth, self.default_box_size[4] * (self.classes_size + 4), (3, 3), (1, 1), padding='SAME',
                          name='fifthout')
        outsixth = conv2d(netsixth, self.default_box_size[5] * (self.classes_size + 4), (3, 3), (1, 1), padding='SAME',
                          name='sixthout')
        self.feature_maps = [outfirst, outsecond, outthird, outfourth, outfifth, outsixth]
    ...
    def __denseblockpl(self, x, steps=256, firstchannel=256, training=True, name='densepl'):
        with tf.variable_scope(name):
            input = tf.identity(x, name="INPUTS")
            netbn2 = max_pooling2d(input, (2, 2), (2, 2), padding='SAME', name='bnpool2')
            netbn2 = batch_normalization(netbn2, training=training, trainable=True, name=name + 'bn2pl')
            netbn2 = tf.nn.relu(netbn2)
            netbn2 = conv2d(netbn2, firstchannel, (1, 1), (1, 1), padding='SAME', name='bnconv2')
            netbn = batch_normalization(input, training=training, trainable=True, name='bn')
            netbn = tf.nn.relu(netbn)
            net = conv2d(netbn, firstchannel, (1, 1), (1, 1), padding='SAME', name='neta')
            netbn = batch_normalization(net, training=training, trainable=True, name='bn2')
            netbn = tf.nn.relu(netbn)
            net = conv2d(netbn, steps, (3, 3), (2, 2), padding='SAME', name='netb')
            nettemp = tf.concat([net, netbn2], -1, name='concat')
        return nettemp

    def __denseblockfin(self, input, step=256, firstchannel=256, training=True, name='densepl'):
        with tf.variable_scope(name):
            input = tf.identity(input, name="INPUTS")
            netbn2 = max_pooling2d(input, (3, 3), (1, 1), padding='VALID', name='bnpool2')
            netbn2 = batch_normalization(netbn2, training=training, trainable=True, name=name + 'bn2pl')
            netbn2 = tf.nn.relu(netbn2)
            netbn2 = conv2d(netbn2, firstchannel, (1, 1), (1, 1), padding='SAME', name='bnconv2')
            netbn = batch_normalization(input, training=training, trainable=True, name='bn')
    ...
    def __stem_block(self, x, filters, kernel_size, stride, padding, is_train, name):
        x = conv2d(x, filters, kernel_size, stride, padding=padding, name=name + "/conv")
        x = batch_normalization(x, trainable=True, training=is_train, name=name + "/bn")
        x = tf.nn.relu(x)
        return x

    def __conv_bn_relu(self, x, conv_filters, conv_size, conv_strides, pool_size, pool_strides, is_training, name):
        x = batch_normalization(x, trainable=True, training=is_training, name=name + "/bn1")
        x = tf.nn.relu(x, name=name + "/relu1")
        x = conv2d(x, filters=conv_filters, strides=conv_strides, kernel_size=conv_size, padding='SAME',
                   name=name + '/conv')
        x = max_pooling2d(x, pool_size=pool_size, strides=pool_strides, name=name + "/pooling")
        return x

    def __denseblock(self, x, nblock=1, steps=48, firstchannel=128, is_train=True, name='denseblock'):
        with tf.variable_scope(name):
            temp = tf.identity(x, name='inputs')
            for i in range(nblock):
                bn = batch_normalization(temp, trainable=True, training=is_train, name='bn/' + str(i))
                net = conv2d(bn, firstchannel, 1, 1, padding='SAME', name='neta/' + str(i))
                bn = batch_normalization(net, training=is_train, trainable=True, name='bn2/' + str(i))
                net = conv2d(bn, steps, 3, 1, padding='SAME', name='netb/' + str(i))
                nettemp = tf.concat([temp, net], -1, name='concat/' + str(i))
                net = nettemp
        return net
